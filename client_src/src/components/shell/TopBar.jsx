  ...
  return (
    <header className="flex h-12 shrink-0 items-center gap-4 border-b border-pramaan-border bg-pramaan-bg px-4">
      ...
      <div className="mx-auto hidden w-full max-w-xl items-center gap-1 rounded-md border border-pramaan-border bg-pramaan-surface px-1 py-1 md:flex">
        ...
        <input
          placeholder={mode === 'search' ? 'Search entities, cases, phones, vehicles…' : 'Ask: "Who connects PRM-4821 to Vellum LLC?"'}
      ...
      <div className="ml-auto flex items-center gap-1.5 md:ml-0">
        ...
        <button className="flex items-center gap-1.5 rounded-md bg-pramaan-primary px-3 py-1.5 text-pramaan-text transition-colors hover:bg-pramaan-secondary" style={type.label}>
...
function ModeBtn({ on, onClick, icon: Icon, label }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 px-2.5 py-1 transition-colors ${on ? 'bg-pramaan-primary/20 text-pramaan-secondary' : 'text-pramaan-text-secondary hover:text-pramaan-text'}`}
      style={type.label}
    >
