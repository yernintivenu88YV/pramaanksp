...
import {
  ...
  Waypoints, Clock, Fingerprint, CopyCheck, ScrollText,
      ...
      <div className="flex h-[72px] shrink-0 items-center gap-2.5 border-b border-pramaan-border px-3.5">
        ...
        {!collapsed && (
          <div className="min-w-0 leading-tight">
            ...
            <div className="truncate text-pramaan-text-secondary" style={type.micro}>Investigation OS · v3.1</div>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-3">
        {groups.map((g) => (
          <div key={g.heading} className="mb-4 px-2.5 last:mb-0">
            {!collapsed && (
              <div className="px-2 pb-1.5 text-pramaan-text-secondary/50 uppercase" style={type.eyebrow}>{g.heading}</div>
            )}
                ...
                return (
                  <button
                    ...
                    className={`group relative flex items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left transition-colors ${
                      on ? 'bg-pramaan-primary/12 text-pramaan-text' : 'text-pramaan-text-secondary hover:bg-pramaan-elevated hover:text-pramaan-text'
                    }`}
                    style={type.label}
                  >
                    ...
                    {!collapsed && badge != null && (
                      <span className="tnum rounded bg-pramaan-panel px-1.5 text-pramaan-text-secondary" style={type.micro}>{badge}</span>
                    )}
                  </button>
                );
              })}
        ...
        <div className={`flex items-center gap-2.5 rounded-md px-1 py-1 ${collapsed ? 'justify-center' : ''}`}>
          ...
          {!collapsed && (
            <div className="min-w-0 flex-1 leading-tight">
              <div className="truncate text-pramaan-text" style={type.label}>A. Okonkwo</div>
              <div className="truncate text-pramaan-text-secondary" style={type.micro}>Lead Analyst · Clearance 4</div>
            </div>
          )}
          {!collapsed && (
