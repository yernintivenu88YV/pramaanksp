...
/* 32px always-visible footer. Holds low-emphasis system status that would
   otherwise compete with navigation: connection, background sync, language,
   secure-session identity, clock. */
export function StatusBar({ syncing }: { syncing: boolean }) {
  ...
  return (
    <footer className="tnum flex h-8 shrink-0 items-center gap-4 border-t border-pramaan-border bg-pramaan-bg px-4 font-mono text-[9px] uppercase tracking-[0.08em] text-pramaan-text-secondary">
      <span className="flex items-center gap-1.5">
        <ShieldCheck size={11} className="text-pramaan-success" />
        Secure session · BLR-CCU-04
      </span>
      <span className="h-3 border-l border-pramaan-border" />
      <span className="flex items-center gap-1.5">
        <Wifi size={11} className="text-pramaan-success" />
        Online
      </span>
      <span className="flex items-center gap-1.5">
